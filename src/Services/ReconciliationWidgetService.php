...
class ReconciliationWidgetService
{
   public static function getStatsData(): array
   {
       ...
       $todayReconciliation = Reconciliation::dateSum('provision_date', $today,'total_amount');
       ...
       $weekReconciliation = Reconciliation::betweenDateSum('provision_date', $weekAgo, $today, 'total_amount');
       ...
       $matchingRecords = 0;
       if ($totalRecords > 0) {
           $reconciliations = Reconciliation::select('provision_date','park_id','total_amount')->get();
           if ($reconciliations->isNotEmpty()) {
               ...
               $minDate = $dates->min();
               $maxDate = $dates->max();
               $payments = Reconciliation::getPaymentsForReconciliation($parkIds, $minDate, $maxDate);
